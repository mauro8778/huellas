  ...
  async updateShelterGeocode(shelterId: string, address: string): Promise<any> {
    try {
      ...
      if (!geocodeData || !geocodeData.lat || !geocodeData.lon || !geocodeData.display_name) {
        throw new HttpException('Datos de geocodificación no válidos', 400);
      }
      ...
      console.error('Error updating shelter geocode:', error.message);
      throw new HttpException(`Error al actualizar la geocodificación del refugio: ${error.message}`, error.response?.status || 500);
    }
  }
}
