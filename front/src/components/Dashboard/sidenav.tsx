...
//             role: decodedToken.role,  // Asumiendo que el token incluye el rol
//           });
//           setIsLoggedIn(true);
...
//* USO EXCLUSIVO PARA SIMULAR UNA SESION DE ADMINISTRADOR O USUARIO CODIGO ORIGINAL ARRIBA  */
// SideNav.tsx
'use client';
import { useEffect, useState } from 'react';
import Link from 'next/link';
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      ...
      if (session) {
        const { id_token } = JSON.parse(session);
        if (id_token) {
          const decodedToken = JSON.parse(id_token);
          if (decodedToken) {
            setUserData({
              name: decodedToken.name,
              nickname: decodedToken.nickname,
